...
function ArticleFeatureCard({ article }: { article: ArticleMeta }) {
  return (
    <article className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg hover:shadow-blue-100">
      <ArticleImage article={article} size="large" />
      <div className="p-7">
        <Link
...
function CompactArticleCard({ article }: { article: ArticleMeta }) {
  return (
    <article className="grid gap-4 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-blue-200 hover:shadow-md hover:shadow-blue-100 sm:grid-cols-[120px_1fr]">
      <ArticleImage article={article} size="compact" />
      <div>
...
function ArticleImage({
  ...
  return (
    <Link
      ...
      className={`grid place-items-center bg-gradient-to-br from-blue-50 via-white to-slate-100 text-xs font-black text-blue-700 ${
        size === "large" ? "aspect-[16/9] w-full" : "aspect-[4/3] w-full rounded-2xl"
      }`}
